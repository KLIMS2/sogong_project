...
(회원가입 기능 완성 커밋)
- 폼에 값이 입력되지 않았을 때의 오류 메시지 변경
- 아이디, 비밀번호의 크기 제한 설정
- 아이디 중복 확인 기능 구현

(회원가입 테스트 커밋)
- MockMvc 클래스를 이용하여 get 요청을 보냈을 때 테스트
- post 요청을 보냈을 때 테스트
- post 요청시 유효성 검사가 작동하는지 테스트
- 아이디 중복 확인 post 요청시 아이디 중복 검사가 작동하는지 테스트
- DB에 회원가입이 제대로 됐는지 테스트

(회원가입 기능 수정 커밋)
- 아이디 중복 확인 기능을 회원가입 폼과 같은 경로를 이용하게 변경
- Rq 클래스 생성
- Rq에 html 출력 관련 함수들 구현
- 회원가입 성공시 alert 창이 뜨고 "/member/login" 경로로 이동
- 회원가입 테스트 수정
- 뒤로가기로 회원가입 폼에 접근시 "/" 경로로 이동하여 회원가입 폼에 접근하지 못하게 방지

(로그인 뒤로가기 보안 커밋)
- 마이페이지에서 뒤로가기 시 로그인 폼이 나오는 문제를 JS를 통해 임시 해결

(회원가입 프론트엔드 유효성 검사 커밋)
- 회원가입 페이지에서 유효성 검사 복구

(게시판 관련 수정 커밋)
- 게시판 생성 시 값의 유무나 제목의 길이 제한과 관련된 유효성 검사를 백엔드에서 구현
- error.html에서 중복된 코드 개선
- 게시판 생성 후 "/board/detail/{id}" 경로로 리다이렉트
- 게시판 상세보기 경로로 접근 시 게시판 상세보기 페이지 출력
- 게시판 상세보기 시 게시판이 없는 경우 처리
- 게시판 생성 후 뒤로가기로 게시판 생성 폼에 접근하는 것을 방지

(회원가입 수정 커밋)
- 회원가입 로직 최적화
- 회원가입 후 alert 창이 아닌 알림 창으로 변경
- 아이디 중복 확인 시 비밀번호도 입력해야 하는 문제 해결

(회원가입 폼 수정 커밋)
- 프론트엔드에서 아이디, 비밀번호 등의 길이 확인
- 아이디 중복 표시나 오류 표시가 폼을 제출하지 않고 값을 변경하자마자 사라지게 변경

(코드 최적화 커밋)
- Util 클래스 생성
- Util에 특정 클래스에서 특정 필드의 annotaion의 값을 가져오는 기능 구현
- Util에 특정 클래스에 있는 모든 필드들의 이름을 가져오는 기능 구현
- Rq에 폼 클래스에 있는 어노테이션의 값을 가져와 모델에 담는 기능 구현
- Util이 제대로 작동하나 테스트
- Rq를 이용해 회원가입 폼에서 입력값의 길이를 비교할 때 모델의 값을 이용
- 폼 클래스에서 Size 어노테이션의 최소/최대 값을 바꾸어도 일일이 바꿀 필요를 제거
- Rq에 캐시 시스템을 도입하여 한 번 어노테이션의 값을 가져오면 다시 가져오지 않고 캐시 데이터를 이용하여 성능 최적화